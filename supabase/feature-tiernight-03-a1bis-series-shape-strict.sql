-- =============================================================================
-- FEATURE-TIERNIGHT-03-A1-bis — Shape série : types JSON stricts
-- =============================================================================
-- Prérequis : A1 (ou A) déjà appliqué, ou greenfield après SERIES-03A.
-- Idempotent : CREATE OR REPLACE + REVOKE.
--
-- Durcissements vs A1 :
--   1. topicSnapshot.id / name = string JSON (jsonb_typeof) puis trim non vide
--   2. custom : bool JSON | legacy "true"|"t"|"false"|"f" uniquement
--      absent = legacy false (officiel) ; null = malformé ; wire custom sans flag = reject
--   3. ledgers : chaque élément string JSON → TNS_LEDGER_INVALID_ENTRY
--   4. categoryIds : non vide, strings non vides, uniques, soit ["*"] seul soit cats
--      (appartenance catalogue = JS uniquement)
--   5. TNS_SHAPE_EXCEPTION reste filet uniquement
--
-- Ordre : … → A1 → CE fichier (A1-bis)
-- =============================================================================
...
comment on function public.tiernight_series_validate_series_shape(jsonb, text) is
  'FEATURE-TIERNIGHT-03-A1-bis: shape totale + types JSON stricts (id/name/custom/ledgers/categoryIds).';
