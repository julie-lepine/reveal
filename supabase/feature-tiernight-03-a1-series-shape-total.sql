-- =============================================================================
-- FEATURE-TIERNIGHT-03-A1 — Validateur série TOTAL (jamais d'exception non mappée)
-- =============================================================================
-- ⚠️ Suite obligatoire : feature-tiernight-03-a1bis-series-shape-strict.sql
--    (types id/name/custom/ledgers/categoryIds stricts).
---- Prérequis : SERIES-03A helpers présents ; idéalement 03-A déjà appliqué.
-- Idempotent : CREATE OR REPLACE + REVOKE helpers.
--
-- Objectif « TOTAL » :
--   Entrée invalide / cast impossible / surprise runtime
--   → TOUJOURS jsonb { "ok": false, "code": "TNS_..." }
--   → JAMAIS raise vers l'appelant (finalize/advance convertissent ensuite).
--
-- Corrections vs feature-tiernight-03-series-contract.sql (03-A) :
--   1. Casts version / roundIndex entry protégés
--   2. categoryIds doit être un tableau JSON
--   3. Flag custom : booléen JSON natif ou string "true"/"t"
--   4. Filet exception outer → TNS_SHAPE_EXCEPTION (sans dump sensible)
--   5. ACL : REVOKE public/anon/authenticated (helper interne, comme 03A)
--
-- Ordre :
--   … → 03a-hardening → (05 advance) → 03-series-contract (A) → CE fichier (A1)
--   Greenfield : A1 suffit pour remplacer la shape (pas besoin de rejouer A).
-- =============================================================================
...
declare
  ...
  v_cats jsonb;
begin
  -- Filet total : toute surprise → code TNS_*, jamais raise
  begin
    ...
    for v_i in 0 .. v_round_count - 1 loop
      ...
      if v_custom_node is null or jsonb_typeof(v_custom_node) = 'null' then
        ...
        v_snap_custom := lower(trim(coalesce(v_snap ->> 'custom', ''))) in ('true', 't');
      else
        -- nombre / objet / array : incohérent pour un flag custom
        return jsonb_build_object(
          'ok', false,
  ...
  exception when others then
    -- Totalité : aucune exception ne doit sortir de cette fonction.
...
comment on function public.tiernight_series_validate_series_shape(jsonb, text) is
  'FEATURE-TIERNIGHT-03-A1: shape série TOTAL — counts 3/5/8 (+7 legacy) ; customs OK ; jamais raise.';

-- Helper interne : pas d'EXECUTE client (aligné SERIES-03A)
