-- =============================================================================
-- FEATURE-TIERNIGHT-04E — ready invité sur prep Rank Live
-- =============================================================================
-- Étend contribute_game_session_player (BUG-TIERNIGHT-PREP-GUEST-01) pour :
--   screen='tiernight-prep'      → ready / poolInvalidate sous tierNightPrep
--   screen='tiernight-live-prep' → ready sous tierNightLivePrep
--
-- Discriminant = screen strict (pas de LIKE).
-- setupEpoch lu dans le blob prep correspondant au screen.
-- pool_invalidate_request reste roster-only (customRosterTopics + tierNightPrep).
--
-- Ne fusionne PAS avec start_tiernight_live_series — gate terrain séparé.
-- Prérequis : feature-tiernight-03-prep-guest-contribute.sql (ou équivalent).
-- =============================================================================
...
begin
  ...
  -- -------------------------------------------------------------------------
  -- TierNight prep (roster + live) : isolation STRICTE
  -- Ready : tiernight-prep | tiernight-live-prep
  -- pool_invalidate : roster only (tiernight-prep)
  -- -------------------------------------------------------------------------
  ...
  if v_kind = 'ready' then
    ...
    -- Roster-only : { requestId, customEntryId } sous customRosterTopics
    if jsonb_typeof(p_value) <> 'object' then
...
comment on function public.contribute_game_session_player(uuid, text, text, jsonb) is
  'Contributions joueur ; TierNight ready → tierNightPrep|tierNightLivePrep selon screen strict.';
