...
| **M-06a** | Exit invité : pas `endGameSession` | `exitGame.js` — `exitGameToGameSelect()` | Quitter mid-game | Autres continuent (voulu) mais état local stale | Chemin invité aligné sur `returnToGameSelect()` : suppress + reset blobs locaux ; bouton Rejoindre robuste via délégation + cible embarquée | 🟡 Corrigé + tests ; QA rejoin banner à refaire |
...
| **SYN-28** / **L-nav** | `navAccess` suppress 15 min scores | `navAccess.js`, `gameSync.js` | Invité consulte classement, hôte relance | Pas de reroute nouvelle prep | 🟡 Partiellement corrigé + tests ; QA à faire : suppress lié à la signature de session, suivi dès que l'hôte avance |
