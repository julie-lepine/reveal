...
| **M-06a** | Exit invité : pas `endGameSession` | `exitGame.js` — `exitGameToGameSelect()` | Quitter mid-game | Autres continuent (voulu) mais état local stale | Chemin invité aligné sur `returnToGameSelect()` : suppress + reset blobs locaux ; bouton Rejoindre robuste via délégation | 🟡 Corrigé + tests ; QA rejoin banner à refaire |
